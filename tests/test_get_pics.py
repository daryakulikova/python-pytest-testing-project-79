...
from hexlet_code.page_loader import dwl_pics_mod_html, make_name, make_dir
import shutil
import os
...
def test_get_pics():
    with tempfile.TemporaryDirectory() as tmpdirname:
        copyed_html = shutil.copy(
            "tests/fixtures/hexlet.html",
            tmpdirname,
        )
        with requests_mock.Mocker() as m:
            ...
            assert os.path.isfile(
                os.path.join(
                    dir_abs_path, "ru-hexlet-io-assets-professions-python.png"
                )
            )
            with open(copyed_html, "r") as f:
                with open(
                    "tests/fixtures/hexlet_pic.html",
                    "r",
                ) as r:
